...
// Helper to get region entry
function getRegionEntry(data, region, year, month) {
    return data.find(d => d.source_location === region && d.year === year && d.month === month);
}

// Helper to calculate ranks
function getRegionRanks(data, region, year, month) {
    const monthEntries = data.filter(d => d.year === year && d.month === month);
    ...
    return {
        crimeRank: crimeRanked.findIndex(d => d.source_location === region) + 1,
        sentimentRank: sentimentRanked.findIndex(d => d.source_location === region) + 1
    };
}

// Helper to get status
function getStatus(crimeRank) {
    if (crimeRank >= 1 && crimeRank <= 30) {
        return { color: 'error', text: 'High Crime Rate' };
    } else if (crimeRank >= 31 && crimeRank <= 75) {
        return { color: 'yellow', text: 'Relatively High Crime' };
    } else if (crimeRank >= 76 && crimeRank <= 150) {
        return { color: 'warning', text: 'Relatively Safe' };
    } else {
        return { color: 'success', text: 'Safe' };
    }
}

// Helper to get reasons
function getCrimeReasons(entry) {
                    ...
                    <Box className="dashboard-main" sx={{ display: 'flex', flexDirection: 'row', gap: 2 }}>
                        ...
                        <Box sx={{ flex: 0.3, minWidth: 0, mb: 5 }}>
                            <Card sx={{ height: '100%', overflow: 'auto', p: 2 }}>
                                ...
                                {(() => {
                                    const entry = getRegionEntry(regionalData, selectedRegion, selectedYear, selectedMonth);
                                    if (!entry) return <Typography>No data for this region/month.</Typography>;
                                    const { crimeRank, sentimentRank } = getRegionRanks(regionalData, selectedRegion, selectedYear, selectedMonth);
                                    const { color: statusColor, text: statusText } = getStatus(crimeRank);
                                    const reasons = getCrimeReasons(entry);
